    ...
    return (
        <div>
            <Table>
                ...
                <TableHeader>
                    <TableRow>
                        <TableHead className="text-gray-400 font-medium">FullName</TableHead>
                        ...
                        <TableHead className="text-gray-400 font-medium">Resume</TableHead>
                ...
                <TableBody>
                    {
                        applicants && applicants?.applications?.map((item) => (
                            <tr key={item._id}>
                                <TableCell className="text-white font-medium">{item?.applicant?.fullname}</TableCell>
                                ...
                                <TableCell >
                                    {
                                        item.applicant?.profile?.resume ? <a className="text-blue-400 cursor-pointer" href={item?.applicant?.profile?.resume} target="_blank" rel="noopener noreferrer">{item?.applicant?.profile?.resumeOriginalName}</a> : <span>NA</span>
                                    }
                                </TableCell>
                                <TableCell className="text-white ">{item?.applicant.createdAt.split("T")[0]}</TableCell>
                                <TableCell className="float-right text-white  cursor-pointer">
                                    <Popover>
                                        <PopoverTrigger>
                                            <MoreHorizontal />
                                        </PopoverTrigger>
